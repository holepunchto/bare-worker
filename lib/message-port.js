...
module.exports = exports = class MessagePort extends EventEmitter {
  constructor(channel) {
    ...
    // A binary duplex stream multiplexed over the same channel as the port's
    // structured messages, fulfilling the conventional `Bare.IPC` API.
  ...
  _refed() {
    return ((this._state | this._ipc._state) & constants.state.REFED) !== 0
  }

  _ref() {
    if (this._port === null) return

    if (this._inflight > 0 || this._refed()) {
      this._port.ref()
    }
  }
    ...
    for await (const message of this._port) {
      switch (message.type) {
        ...
        case constants.message.TERMINATE:
          this._onterminate()
          break
        case constants.message.IPC_DATA:
          this._ipc.push(message.value)
          break
        case constants.message.IPC_END:
          this._ipc.push(null)
          break
      }
    }
